  ...
  */
	...
	private function add_rule($allow,$roles,$resources,$privileges,$assertion)
	{
		...
		if($resources !== NULL)
		{
			...
			{
				if($resource instanceof Simple_Modeler)
				{
		...
		if($roles !== NULL)
		{
			...
			{
				if($role instanceof Simple_User)
				{
					$role = $role->get_roles();
				}
				else
	...
	public function is_allowed($role = NULL, $resource = NULL, $privilege = NULL)
	{
		...
		// normalize role
		$roles = $role !== NULL ? ($role instanceof Simple_User ? $role->get_roles() : (is_array($role) ? $role : (string) $role)) : NULL;
		...
		$resource = $resource !== NULL ? ($resource instanceof Simple_Modeler ? $resource->get_table_name() : (string) $resource) : NULL;
